...
class Video(models.Model):
    src = models.FileField(upload_to='static/videos/reel', null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
class Comment(models.Model):
    text = models.TextField(max_length=5000, default="")
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='reel_comment_user')
...
class Reel(models.Model):
    ...
    videos = models.ManyToManyField(Video, through='ReelVideo')
    ...
    tags = models.ManyToManyField(Tag,through='ReelTag',related_name='reel_tag')
    
    seen = models.ManyToManyField(User,through='ReelSeen', related_name='reel_seen')
